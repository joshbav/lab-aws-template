...
echo
echo INSTALL DOCKER
sudo yum install -y docker-ce-18.09.1-3.el7systemctl enable docker && sudo systemctl start docker && sudo groupadd docker && sudo usermod -aG docker centos && sudo docker run hello-world
